...
def decoder(input_channel, output_channel, num=3):
    # 3与2的原因是后面解码器部分前两个三层 后两个两层，都没有激活函数
    if num == 3:
...
class SegNet(torch.nn.Module):
    def __init__(self):
        ...
        # 用了反池化，所以要在这里存储一下索引，反池化还需要要恢复的尺寸大小
        pool_list = [4, 9, 16, 23, 30]
        for index in pool_list:
            vgg16_pretrained.features[index].return_indices = True  # 这里要修改存一下索引，默认是FALSE

        self.encoder1 = vgg16_pretrained.features[:4]
        self.pool1 = vgg16_pretrained.features[4]   # 这里要单拿出来，返回index
