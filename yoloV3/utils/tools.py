...
def iou_xywh_numpy(boxes1, boxes2):
    """ 计算交并比 IOU
    boxes1和boxes2的shape可以不相同，但是需要满足广播机制
    且需要保证最后一维为坐标维，以及坐标的存储结构为(x,y,w,h)，其中(x,y)是bbox的中心坐标
    返回boxes1和boxes2的IOU，IOU的shape为boxes1和boxes2广播后的shape[:-1]
    :param boxes1:
    :param boxes2:
    :return:
    """
    ...
    # 计算出boxes1与boxes1相交部分的右下角坐标，前两维取大；左上角坐标、前两维取小，这里利用了numpy的广播机制
    ...
    IOU = 1.0 * inter_area / union_area # 二者之比
    return IOU
...
def GIOU_xywh_torch(boxes1, boxes2):
    """
    # 之所以引用这个GIOU,是为了解决如果边界框不相交就无法进行优化的问题。
    不想交也可以计算损失。 引入了一个包围框。
