...
def quick_vis_boxes(img_mix, bboxes_mix):
  # 可视化
  import matplotlib.pyplot as plt
  import matplotlib.patches as patches
  plt.imshow(img_mix)
  currentAxis = plt.gca()
  for i in range(len(bboxes_mix)):
    rect = patches.Rectangle((int(bboxes_mix[i][0]), int(bboxes_mix[i][1])), int(bboxes_mix[i][2] - bboxes_mix[i][0]),
                             int(bboxes_mix[i][3] - bboxes_mix[i][1]), linewidth=1, edgecolor='r', facecolor='none')
