...
class FocalLoss(nn.Module):
    """
    一张图像可能生成成千上万的candidate locations候选框，但是其中只有很少一部分是包含object的，这就带来了类别不均衡。
    所以引用Focal Loss
    """
...
class YoloV3Loss(nn.Module):
    def __init__(self, anchors, strides, iou_threshold_loss=0.5):
        ...
        self.__iou_threshold_loss = iou_threshold_loss  # 0.5
    ...
    def __cal_loss_per_layer(self, p, p_d, label, bboxes, stride):
        ...
        """
        # 这个损失常用于2分类，输入没有进行sigmoid，这里会先进行sigmoid运算，这个BCELoss是处理进行了的输入
        BCE = nn.BCEWithLogitsLoss(reduction="none")
        FOCAL = FocalLoss(gamma=2, alpha=1.0, reduction="none") # 解决类别不平衡的问题
        ...
        label_xywh = label[..., :4]
        label_obj_mask = label[..., 4:5]    # 置信度为1，就相当于取位置了，如果做乘法的话，所以就相当于一个mask
        label_cls = label[..., 6:]
        label_mix = label[..., 5:6]
        ...
        bbox_loss_scale = 2.0 - 1.0 * label_xywh[..., 2:3] * label_xywh[..., 3:4] / (img_size ** 2)     # 目标所占整个图像的大小比例
        loss_giou = label_obj_mask * bbox_loss_scale * (1.0 - giou) * label_mix     # 如果是小目标，loss会特别大。


        # loss confidence
        iou = tools.iou_xywh_torch(p_d_xywh.unsqueeze(4), bboxes.unsqueeze(1).unsqueeze(1).unsqueeze(1))
        iou_max = iou.max(-1, keepdim=True)[0]
        label_noobj_mask = (1.0 - label_obj_mask) * (iou_max < self.__iou_threshold_loss).float()       # 这里是做了一个位置判断，得到iou小于0.5的目标的， 置信度，即可以认为是假例，即没有物体，只产生置信度loss。
        # target是真实值。
        loss_conf = (label_obj_mask * FOCAL(input=p_conf, target=label_obj_mask) +
                    label_noobj_mask * FOCAL(input=p_conf, target=label_obj_mask)) * label_mix      # mix是因为有混合物体，
        ...
        loss_giou = (torch.sum(loss_giou)) / batch_size     # 计算所有格子下的平均
