...
from modellib.Proposal import proposal_layer
from modellib.Detection import detection_layer,detection_target_layer
from modellib.MaskRCNN import MaskRCNN
from datasets.Dataset import Dataset
from engines.losses import compute_losses


class Engine(MaskRCNN):
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.log_dir = os.path.join(self.config.DEFAULT_LOGS_DIR, "{}{:%Y%m%dT%H%M}".format(
    ...
    def find_last(self):
        ...
        dir_names = next(os.walk(self.config.DEFAULT_LOGS_DIR))[1]
        ...
        if not dir_names:
            return None, None
        # Pick last directory
        dir_name = os.path.join(self.config.DEFAULT_LOGS_DIR, dir_names[-1])
