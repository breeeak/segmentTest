...
def main(args):
    ...
    dataset_train = pmr.datasets(args.dataset, args.data_dir, "train", train=True)  # 根据所选的数据集类型来声明训练数据集
    ...
    model = pmr.maskrcnn_resnet50(True, num_classes).to(device)  # 新建模型，并读入预训练好的模型
    ...
    lr_lambda = lambda x: 0.1 ** bisect.bisect([22, 26], x)  # 用于随着epoch的变化改变lr
    
    start_epoch = 0
    prefix, ext = os.path.splitext(args.ckpt_path)  # 将ckpt的地址分为前缀和名称
    ckpts = glob.glob(prefix + "-*" + ext)  # 搜索所有匹配ext的ckpt的名称
    ckpts.sort(key=lambda x: int(re.search(r"-(\d+){}".format(ext), os.path.split(x)[1]).group(1)))  # 按照ckpt中的数值来排序
    if ckpts:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        B = time.time()  # 统计`evaluate`函数的运行时间
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=5)  # 训练的epoch数
